...
c){var d,e=k+a.valueOf();if(D>0&&c.t1<v.t1){c.advance(D);if(j){p=p+void 0;j.intervals.length>4&&j.intervals.splice(0,j.intervals.length-4);if(j.intervals.length>0){j.intervals[0]=j.intervals[0]+void 0;j.intervals[3]=c.rate.valueOf()}}}if(c.t1>G)n&&C.add(n);else{if(j&&c.t1r<=e){j.endTime=e;var l=c.rate.valueOf();for(d=Math.max(0.0010,t*l);m<c.t2r;){j.intervals.push(p);j.intervals.push(m);j.intervals.push(m+d);j.intervals.push(l);m=m+d}}if(c.t2r<e){c.tick();n&&C.add(n)}else{if(j)j.end=true;c.t2r>c.t1r?
g(b,c.nudgeToRel(e),f):g(b,c,f)}}}function n(a){h(a,e,f)}var k=e.t1r,m=k,p=e.t1,l,j;if(b){l=function(a,d){if(d.intervals.length>0){var f=d.intervals[0],e=d.intervals[1],g=d.intervals[2],h=d.intervals[3];if(e<=d.endTime){if(f<N+a){b(d.clock,e,g,d.startTime,d.endTime,h);d.intervals.splice(0,4)}c(l,d);return}}d.end||c(l,d)};j={clock:e,intervals:[],startTime:e.t1r,endTime:e.t1r+a.valueOf(),end:false};c(l,j)}h(d,e)}};m.log=function(a){return O(function(){console.log(a)})};m.anim=function(a,b){var c,d,
f,e;switch(arguments.length){case 3:e=arguments[2];break;case 4:c=arguments[2];d=arguments[3];e=function(a){return(1-a)*c.valueOf()+a*d.valueOf()};break;case 5:c=arguments[2];d=arguments[3];f=arguments[4];e=function(a){a=f(a);return(1-a)*c.valueOf()+a*d.valueOf()};break;default:throw Error("Invalid arguments to anim()");}return a.constructor.name==="AudioGain"||a.constructor.name==="AudioParam"?k(b,function(b,c,d,f,g){var g=g-f,h;if(c<=f){h=g>q?(c-f)/g:0;a.setValueAtTime(e(h),b.rel2abs(c))}a.linearRampToValueAtTime(e(g>
...
i);e=0;for(g=h;e<g;++e)c[e]=d.outputBuffer.getChannelData(e).subarray(q,i);e=0;for(g=x.length;e<g;++e)a[x[e]]=d.inputBuffer.getChannelData(o+e).subarray(q,i);a.playbackTime=(d.playbackTime||k.currentTime)+q/k.sampleRate;a.samplesToStop=P-l;e=j.onaudioprocess.call(y,a);void 0===e&&(e=m-l)}l+e<m&&(1<=LOG_LEVEL&&console.log("models/jsnode.js[172]:\t","Finished",m,P),f=!0,setTimeout(Q,Math.round(1E3*n/k.sampleRate)))}};var R=g||k.destination;Q=function(){f=!0;m.disconnect();g&&g.disconnect();L&&(L.stop(0),
L.disconnect());e&&e.disconnect();y.drop(m);y.emit&&y.emit("ended")};var I;y.prepareAheadTime=0.1;y.start=function(a){if(!d&&!f)if(a){var b=Math.max(a,k.currentTime)-k.currentTime;I&&(cancelTimeout(I),I=null);var c=function(a){p=Math.floor(a*k.sampleRate);d=true;I=null;m.connect(R)};b<=y.prepareAheadTime?c(a):I=setTimeout(c,Math.round(1E3*(b-y.prepareAheadTime)),a)}else d=!0,m.connect(R)};y.stop=function(a){f||(P=Math.max(p,Math.ceil(a*k.sampleRate)))};0<o&&y.start(0);return y};return u});
